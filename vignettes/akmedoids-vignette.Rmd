...
```{r, echo=FALSE, include=FALSE}
col1 <- c("1", "2","3","4", "5", "6")
col2 <- c("`dataImputation`","`rates`", "`props`", "`outlierDetect`","`wSpaces`", "`removeRowsN`")
...
crime_per_200_people <- crime_per_200_people$rates_data
...
Table 2 shows the two main functions required to carry out the longitudinal clustering and generate the descriptive statistics of the resulting groups. The relevant functions are `akClustr` and `statPrint`. The `akClustr` function clusters trajectories according to the similarities of their long-term trends, while the `statPrint` function extracts descriptive and change statistics for each of the clusters. The latter also generates `quality` plots for the best cluster solution.
...
col1 <- c("1", "2")
col2 <- c("`akClustr`","`statPrint`")
col3 <- c("`Anchored k-medoids clustering`","`Descriptive (Change) statistics and plots`")
col4 <- c("Clusters trajectories into a `k` number of groups according to the similarities in their long-term trend and determines the best solution based on the Calinski-Harabasz criterion","Generates the descriptive and change statistics of groups, and also plots the groups performances")
...
In the following sections, we provide a worked example of clustering with `akClustr` function using the `prop_crime_per200_people` object. The `statPrint` function will then generate the descriptive summary of the clusters. The `prop_crime_per200_people` object is plotted in `r fig$ref("figs5")` as follows: 
...
#clustering
quality_plot <- akClustr(prop_crime_per200_people, id_field = TRUE, 
...
In addition to the performance plot (Figure `r fig$ref("figs6")`) that shows the `Calinki-Harabasz` scores at different values of `k`, the `akClustr` function also generates output messages which include the group memberships of the best solution, as determined quality criterion used. From the plot, the best value of `k` is the highest at `k=5`, and therefore determined as the best solution. It is recommended that the determination based on either of the quality criteria should be used complementarily with users judgment in relation to the problem at hand. 
...
#clustering
cluster_output <- akClustr(prop_crime_per200_people, id_field = TRUE, 
                                  method = "linear", k = c(5))

#vector of group memberships
as.vector(cluster_output$memberships$alphabetic_Labels) 
...
clustr <- as.vector(cluster_output$memberships$alphabetic_Labels) 
clustr
...
By changing the argument `type="line"` to `type="stacked"`, a `quality plot` is generated instead (see Figure `r fig$ref("figs9")`). Note that these plots make use of functions within the `ggplot2` library [@Wickham2016]. For a more customized visualisation, we recommend that users deploy the `ggplot2` library directly. 
