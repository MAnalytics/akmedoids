...
The properties (i.e. the descriptive and change statistics) of any of the cluster solutions (i.e. at any value of `k`) can be accessed using the special `print` function `print_akStats`. The print function takes the `akObject` class generated by the `akmedoids::akClustr` function as input as follows: 

```{r, echo=TRUE, message=FALSE, eval=TRUE}

#Specifying the optimal solution, output$optimal_k (i.e. `k = 3`)

prpties = print_akStats(output, k = , type="lines", y.scaling="fixed", show_Plots = FALSE)
...
* output Variables described as follow:
  ...
  + `descriptiveStats` - descriptive statistics of the cluster.
  
  + `changeStats` - change statistics of the cluster
  
  + `cluster_Plot` - the line plot or stacked histogram of cluster groups.
