#' @title Removes rows that contain 'NA' and 'Inf' entries
#' @description This function removes any rows in which an 'NA' or an 'Inf' entry is found.
#' @param traj [data.frame (numeric)]: longitudinal data. Each row represents an individual trajectory (of observations). The columns show the observations at consecutive time points.
...
#' @details Given a matrix (or a dataframe) containing an 'NA' or an 'Inf' entry, the function returns only rows with complete observations.
