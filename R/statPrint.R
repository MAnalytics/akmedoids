...
#' the \code{\link{akmedoids.clust}} function, and
#' (ii) generates the plots of the groups (performances).
#' @param clustr [vector (character)] A vector of cluster
#' membership (labels). For instance, the result extracted
#' from the \code{\link{akmedoids.clust}} function.
#' @param traj [matrix (numeric)]: corresponding longitudinal
#' data used to generate \code{clustr} (with rows
#' corresponding to each label of \code{clustr}).
#' For example, the first label of \code{clustr}
#' is the group label of the first row of
#' \code{traj} matrix, and so on.
#' @param id_field [numeric or character] Whether the first
#' column of the \code{traj} is a unique (\code{id}) field.
#' Default: \code{FALSE}. If \code{TRUE} the function
#' recognises the second column as the first time step.
...
#' of the trajectory groupings. Given a vector of group
#' membership (labels) and the corresponding data matrix
#' (or data.frame) indexed in the same order, this
#' function generates all the descriptive and
#' change statistics of all the groups.
#' The function can generate a line and an area stacked plot
#' drawing from the functionalities of the \code{ggplot2} library.
...
statPrint <- function(clustr, traj, id_field=TRUE, reference = 1,
                      N.quant = 4, showplots=TRUE, type = "lines",
                      y.scaling="fixed"){
